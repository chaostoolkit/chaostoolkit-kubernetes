...
class MediaPlaneActions(object):
    ...
    def link_specific_list(self, list):
        ...
        if len(list) == 0:
            ...
            if self.none_or_value(list[34]).startswith("2405:0203") or self.none_or_value(list[34]).startswith("2405:0204") or self.none_or_value(list[34]).startswith("2405:0205") or self.none_or_value(list[34]).startswith("2409:4000"):
                return [self.none_or_value(list[34]), self.none_or_value(list[35]), self.none_or_value(list[19]), self.none_or_value(list[20]), self.none_or_value(list[45]), self.none_or_value(list[43]), self.none_or_value(list[11]), self.none_or_value(list[12]), self.none_or_value(list[22]), self.none_or_value(list[26]), self.none_or_value(list[27]), self.none_or_value(list[32]), self.none_or_value(list[33]), self.none_or_value(list[36])]
            ...
            # checked if its uplink record
                ...
                return [self.none_or_value(list[19]), self.none_or_value(list[20]), self.none_or_value(list[34]), self.none_or_value(list[35]), self.none_or_value(list[45]), self.none_or_value(list[43]), self.none_or_value(list[11]), self.none_or_value(list[12]), self.none_or_value(list[22]), self.none_or_value(list[26]), self.none_or_value(list[27]), self.none_or_value(list[32]), self.none_or_value(list[33]), self.none_or_value(list[36])]
    ...
    def validate_media_plane(self, validation_entities):
        hashed_row_dict = {}
        min_time = validation_entities.sqlite_adapter.select("select min(cal_timestamp_time) from %s_input1" % self.job_alias)[-1][0]

        max_time = validation_entities.sqlite_adapter.select("select max(cal_timestamp_time) from %s_input1" % self.job_alias)[-1][0]

        time_range_list = self.comman_action.get_time_range_list(min_time, max_time, self.frequency, date_time_format=self.date_format_usual)
        ...
        for time_range in time_range_list:
            ...
            total_dump = validation_entities.sqlite_adapter.select("select * from %s_input1 where %s" % (self.job_alias, where_clause))
            temp_list = []
            hashed_row_dict = {}
            for row in total_dump[1:]:
                temp_dict = {}
                row = list(row)
                if row[19].startswith("2405:0203") or row[19].startswith("2405:0204") or row[19].startswith("2405:0205") or row[19].startswith("2409:4000"):
                    hash_value = self.generate_hash(row[19], row[20], row[34], row[35])
                elif row[34].startswith("2405:0203") or row[34].startswith("2405:0204") or row[34].startswith("2405:0205") or row[34].startswith("2409:4000"):
            ...
            for hash_value in hashed_row_dict.keys():
                ...
                if len(media_uplink_row) == 0:
                    ...
                    m_binned_timestamp = (re.sub('[^A-Za-z0-9]+', "", self.comman_action.computeBinTime(media_downlink_row[1])))
                    sql_timestamp = self.comman_action.date_format_changer(m_binned_timestamp, "%Y%m%d%H%M%S","%Y-%m-%d %H:%M:%S")
                    ...
                    m_u_rtd_sum = 0
                    m_u_cell_id = -1
                    m_u_calculated_item = [str(m_u_wei_jitter), str(m_u_wei_mos), str(m_u_wei_pakt_los), str(m_u_wei_rtd), str(m_u_jitter_sum), str(m_u_rtd_sum), str(m_u_pakt_los_sum), str(m_u_degradation_sum), str(m_u_cell_id)]
                    m_cell_id = m_u_cell_id

                else:
                    m_timestamp = (re.sub('[^A-Za-z0-9]+', "", media_uplink_row[1]))
                    m_binned_timestamp = (re.sub('[^A-Za-z0-9]+', "", self.comman_action.computeBinTime(media_uplink_row[1])))
                    sql_timestamp = self.comman_action.date_format_changer(m_binned_timestamp, "%Y%m%d%H%M%S","%Y-%m-%d %H:%M:%S")
                    ...
                    m_u_degradation_sum = int(media_uplink_row[13]) + int(media_uplink_row[14]) + int(media_uplink_row[15])
                    m_u_jitter_sum = int(media_uplink_row[23]) + int(media_uplink_row[24]) + int(media_uplink_row[25])
                    m_u_pakt_los_sum = str(int(media_uplink_row[28]) + int(media_uplink_row[29]) + int(media_uplink_row[30]))
                ...
                if len(media_downlink_row) == 0:
                    ...
                    m_d_rtd_sum = 0
                    m_d_cell_id = -1
                    m_d_calculated_item = [str(m_d_wei_jitter), str(m_d_wei_mos), str(m_d_wei_pakt_los), str(m_d_wei_rtd), str(m_d_jitter_sum), str(m_d_rtd_sum), str(m_d_pakt_los_sum), str(m_d_degradation_sum), str(m_d_cell_id)]
                    ...
                    m_d_degradation_sum = int(media_downlink_row[13]) + int(media_downlink_row[14]) + int(media_downlink_row[15])
                    m_d_jitter_sum = int(media_downlink_row[23]) + int(media_downlink_row[24]) + int(media_downlink_row[25])
                    m_d_pakt_los_sum = int(media_downlink_row[28]) + int(media_downlink_row[29]) + int(media_downlink_row[30])
                    m_d_rtd_sum = int(media_downlink_row[16]) + int(media_downlink_row[17]) + int(media_downlink_row[18])
                    m_d_cell_id = self.generate_cell_id(media_downlink_row[6], media_downlink_row[7], media_downlink_row[3])
                    m_d_calculated_item = [str(m_d_wei_jitter), str(m_d_wei_mos), str(m_d_wei_pakt_los), str(m_d_wei_rtd), str(m_d_jitter_sum), str(m_d_rtd_sum), str(m_d_pakt_los_sum), str(m_d_degradation_sum), str(m_d_cell_id)]
                    m_cell_id = m_d_cell_id
